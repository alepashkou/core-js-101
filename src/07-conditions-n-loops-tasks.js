...
function isInsideCircle(circle, point) {
  const points = (point.x - circle.center.x) * (point.x - circle.center.x)
  + (point.y - circle.center.y) * (point.y - circle.center.y);
  if (points < circle.radius * circle.radius) {
    return true;
  }
  return false;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const str = [];
  str.push(isStartIncluded ? '[' : '(');
  str.push(a > b ? b : a);
  str.push(', ');
  str.push(a < b ? b : a);
  str.push(isEndIncluded ? ']' : ')');
  return str.join('');
}
...
function getDigitalRoot(num) {
  const count = String(num).split('').reduce((acc, el) => String(Number(acc) + Number(el)));
  return count > 9 ? Number(count[0]) + Number(count[1]) : count;
}
...
function toNaryString(num, n) {
  const nary = parseInt(num, 10);
  return nary.toString(n);
}
